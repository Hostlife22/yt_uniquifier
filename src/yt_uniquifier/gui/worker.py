...
The Worker contains **no business logic** — it forwards RunEvents from the
core layer as Qt signals so the main window can update widgets on the GUI
thread. This is the lesson learned from the legacy prototype, where the
VideoProcessor(QThread) baked the entire pipeline into the GUI layer.

Soft-imports PyQt6 so the package can still be installed without the [gui]
extra; the module raises on first use only.
"""
...
try:
    ...
    raise ImportError(
        "PyQt6 is not installed. Install the [gui] extra:\n"
...
class Worker(QThread):
    """Wraps orchestrator.run_full and proxies its events as Qt signals.

    Signals carry primitive payloads (str, int, float, dict) only — no
    pydantic models or pathlib.Path — for safe cross-thread emission.
    """

    progress = pyqtSignal(float, str)         # overall fraction 0..1, status message
    ...
    def run(self) -> None:  # noqa: D401 - Qt override
        ...
        self.finished_ok.emit(str(self.options.output), str(qa_html) if qa_html else "")
