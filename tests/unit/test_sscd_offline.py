...
def test_default_model_loader_raises_install_hint_without_torch(
    ...
    """Without torch, the loader must raise the supported-extra hint."""
    import builtins
    ...
    with pytest.raises(PipelineError, match=r"\[ml\]"):
...
def test_ensure_model_cached_returns_verified_existing_file(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    cache_path = tmp_path / "sscd_disc_mixup.torchscript.pt"
    cache_path.write_bytes(b"fake weights")
    monkeypatch.setattr(sscd, "_MODEL_SHA256", hashlib.sha256(b"fake weights").hexdigest())
    assert _ensure_model_cached(tmp_path) == cache_path


def test_ensure_model_cached_rejects_tampered_existing_file(tmp_path: Path) -> None:
    cache_path = tmp_path / "sscd_disc_mixup.torchscript.pt"
    cache_path.write_bytes(b"tampered weights")

    with pytest.raises(PipelineError, match="cached SSCD model SHA-256 mismatch"):
