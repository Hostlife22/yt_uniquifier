...
| P1 | Distributed liveness | **PARTIAL:** autogenerated worker ID включает host+PID+nonce, siblings больше не маскируют друг друга; fencing отсутствует | `core/queue/leasing.py::FileQueue` | Revived stale worker может опубликовать результат после reap | Per-job fencing token before publish/release |
| P1 | Distributed resume | **PARTIAL:** worker сохраняет target VMAF и использует plan-hash work dir; cross-host fencing/QA policy open | `cli/cmd_worker.py::_process_one` | Recovery content-bound, но exactly-once publish не доказан | Stable job ID/fencing и explicit worker QA configuration |
